...
var urltimeout = 3800; // таймаут ожидание ответа от контроллера. Чем хуже интертнет, тем выше значения. Но не более времени обновления параметров
...
function setParam(paramid, resultid) {
	...
	if(rer.test(paramid)) {
		swichid = document.getElementById(elid);
		if(swichid.checked) {
			elval = 1;
		} else {
			elval = 0;
	...
	if(rel.test(paramid)) {
		elsend = elsend.replace(/\(/g, "=").replace(/\-/g, "(");
	}

	if(!rec.test(paramid)) {
		document.getElementById(valueid).value = "";
		document.getElementById(valueid).placeholder = "";
					...
					if(arr != null && arr != 0) {
						...
						for(var i = 0; i < arr.length; i++) { //console.log(arr.length);
							if(arr[i] != null && arr[i] != 0) {
								...
								if(rec.test(arr[i])) {
									var type = "const";
								} else if(rei.test(arr[i])) {
									var type = "sensorip";
								} // 
								else if(res.test(arr[i])) {
									var type = "select";
								} // значения
								else if(reg.test(arr[i])) {
									var type = "chart";
								} // график
								else if(ret.test(arr[i])) {
									var type = "scheduler";
								} // расписание
								else if(reo.test(arr[i])) {
									var type = "scan";
								} // ответ на сканирование
								else if(rep.test(arr[i])) {
									var type = "present";
								} // наличие датчика в конфигурации
								else if(rer.test(arr[i])) {
									var type = "checkbox";
								} // чекбокс
								else if(rev.test(arr[i])) {
									var type = "values";
								} // значения
								else {
									var type = "str";
							...
							opposite: true],*/
									...
									if(values[0] != null && values[0] != 0 && values[1] != null && values[1] != 0) {
										...
										cont3 = values[1].replace(/[0-9]:.{1,10}:[-0-9\.]{1,5}:/g, "");
										cont3 = cont3.replace(/;$/g, "");
										console.log(cont3);
									...
									if(values[0] != null && values[0] != 0 && values[1] != null && values[1] != 0) {
										...
										if(idsel == "get_testmode") {
											...
											for(var i = 0; i < count.length - 1; i++) {
												...
												content = content + '<td><input id="get_zeropress-' + input + '" type="number" min="0" max="2048" step="1" value=""><input type="submit" value=">"  onclick="setParam(\'get_zeroPress(' + count[i] + ')\');"></td>';
												content = content + '<td><input id="get_transpress-' + input + '" type="number" min="0" max="4" step="0.001" value=""><input type="submit" value=">"  onclick="setParam(\'get_transPress(' + count[i] + ')\');"></td>';
												content = content + '<td><input id="get_testpress-' + input + '" type="number" min="-1" max="50" step="0.01" value=""><input type="submit" value=">"  onclick="setParam(\'get_testPress(' + count[i] + ')\');"></td>';
										...
										if(element) {
											document.getElementById(idsel).innerHTML = "";
										}
									...
									if(values[0] != null && values[0] != 0 && values[1] != null && values[1] != 0) {
										cont2 = values[0].replace(/\(/g, "-").replace(/\)/g, "").toLowerCase();
										console.log("sensorip??????????????????????????????????????: " + cont2);
										if(values[0] == 'get_numberIP') {
											console.log("get_numberIP!!");
											content = "";
											content2 = "";
											upsens = "";
											loadsens = "";
											count = Number(values[1]);
											console.log("get_numberIP:" + count);
											for(var i = 1; i < count + 1; i++) {
												...
												content_id = 'get_sensor_ip-' + i + '';
												console.log("content_id: " + content_id);
												id_ip1 = "";
												id_ip1 = id_ip1 + 'get_sensorparamip-' + i + ':sensor_temp';
												id_ip2 = "";
												id_ip2 = id_ip2 + 'get_sensorparamip-' + i + ':stime';
												id_ip3 = "";
												id_ip3 = id_ip3 + 'get_sensorparamip-' + i + ':sensor_ip';
												id_ip4 = "";
												id_ip4 = id_ip4 + 'get_sensorparamip-' + i + ':rssi';
												id_ip5 = "";
												id_ip5 = id_ip5 + 'get_sensorparamip-' + i + ':vcc';
												id_ip6 = "";
												id_ip6 = id_ip6 + 'get_sensorparamip-' + i + ':sensor_count';
												...
												//document.getElementById(id_ip1).style.color = "#A181BB"; document.getElementById(id_ip2).style.color = "#A181BB"; document.getElementById(id_ip3).style.color = "#A181BB"; document.getElementById(id_ip4).style.color = "#A181BB"; document.getElementById(id_ip5).style.color = "#A181BB"; document.getElementById(id_ip6).style.color = "#A181BB";
												// document.getElementById(id_ip7).style.width = "20%"; document.getElementById(id_ip8).style.width = "20%";

												//content2 = content2 + '<tr><td><input type="checkbox" id="get_sensoruseip-'+i+'" onchange="setParam(\'get_sensorUseIP('+i+')\');" ></td>';
												//content2 = content2 + '<td><input type="checkbox" id="get_sensorruleip-'+i+'" onchange="setParam(\'get_sensorRuleIP('+i+')\');" ></td>';
												//content2 = content2 + '<td><select id="get_sensorlistip-'+i+'"  onchange="setParam(\'get_sensorListIP-'+i+'\',\'get_sensorlistip-'+i+'\');"></select></td></tr>';
											}
											...
											for(var i = 0; i < count.length - 1; i++) {
												...
												content = content + '<tr id="get_presentrelay-' + relay + '"><td>' + count[i] + '</td><td id="get_noterelay-' + relay + '"></td><td id="get_pinrelay-' + relay + '"></td><td><input style="display:none" type="checkbox" id="get_relay-' + relay + '" onchange="setParam(\'get_Relay(' + count[i] + ')\');" ><label for="get_relay-' + relay + '"><span id="get_relay-' + relay + '2"></span></label></td>';
												content = content + '</tr>';
											...
											for(var i = 0; i < count.length - 1; i++) {
												...
												content = content + '<tr><td>' + count[i] + '</td><td id="get_noteinput-' + input + '"></td><td id="get_input-' + input + '">-</td> <td><input id="get_alarminput-' + input + '" type="number" min="0" max="1" step="1" value=""><input type="submit" value=">" onclick="setParam(\'get_alarmInput(' + count[i] + ')\');"></td><td><input id="get_testinput-' + input + '" type="number" min="0" max="1" step="1" value=""><input type="submit" value=">"  onclick="setParam(\'get_testInput(' + count[i] + ')\');"></td><td id="get_pininput-' + input + '">-</td><td id="get_typeinput-' + input + '">-</td><td id="get_errcodeinput-' + input + '">-</td>';
												content = content + '</tr>';
											}
											document.getElementById(cont2).innerHTML = content; //console.log("get_numberIP-content " + content);
											updateParam(upsens); //console.log("upsens " + upsens);
											...
											for(var i = 0; i < count.length - 1; i++) {
												input = count[i].toLowerCase();
												console.log("input:" + count[i]);
												loadsens = loadsens + "get_noteTemp(" + count[i] + "),";
												upsens = upsens + "get_fullTemp(" + count[i] + "),";
												console.log("get_fullTemp:" + upsens);
												content_t = "get_fullTemp(" + count[i] + ")";
												console.log("$$$$$$$$$$$$$$$$$$$$$$$content_t:" + content_t);
											...
											console.log("get_numberT-content " + content);
											updateParam(upsens);
											console.log("upsens " + upsens);
											loadParam(loadsens);
											console.log("loadsens: " + loadsens);
											...
											for(var i = 0; i < count.length - 1; i++) {
												...
												content = content + '<td><input id="get_kfflow-' + input + '" type="number" min="0.0" max="1000" step="0.001" value=""><input type="submit" value=">"  onclick="setParam(\'get_kfFlow(' + count[i] + ')\');"></td>';
												content = content + '<td id="get_frflow-' + input + '">-</td>';
												content = content + '<td><input id="get_testflow-' + input + '" type="number" min="0.0" max="1000" step="0.001" value=""><input type="submit" value=">"  onclick="setParam(\'get_testFlow(' + count[i] + ')\');"></td>';
											...
											upsens = "";
											loadsens = "";
											count = Number(values[1]); //console.log("get_numberIP:"+count);
									...
									console.log("!!!!!!!!!!!!valueid:" + valueid + " %%%%%%%%%%%%valuevar:" + valuevar);
									...
									console.log("<-!!!!!!!!!!!!!!!!!!!!!->" + type + ": " + values[0] + "<-!!!!!!!!!!!!!!!!!!!!!->" + values[1]);
									...
									if(values[1] == 1) {
										...
										if(element2) {
											element2.innerHTML = '<img src="on.png" width="30%" alt="" />';
										}
										var element4 = document.getElementById(valueid + "3");
										if(element4) {
											document.getElementById(valueid + "3").innerHTML = '<img src="shem.png" alt="">';
											document.getElementById("get_temp-tconoutg_t").style.left = "43%";
										}
										...
										var element2 = document.getElementById(valueid + "2");
										if(element2) {
											element2.innerHTML = '<img src="off.png" width="30%" alt="" />';
										}
										var element4 = document.getElementById(valueid + "3");
										if(element4) {
											document.getElementById(valueid + "3").innerHTML = '<img src="shem1.png" alt="">';
											document.getElementById("get_temp-tconoutg_t").style.left = "17%";
										}
									}
									...
									if(values[0] == "get_WORK" && values[1] == "ON") {
										...
										if(!element) {
											document.getElementById(values[0].toLowerCase()).innerHTML = "ON";
										} else {
										...
										var element = document.getElementById("onoffswitch");
										if(!element) {
											document.getElementById(values[0].toLowerCase()).innerHTML = "OFF";
										} else {
										...
										var valueid = values[0].replace(/set_/g, "get_"). /*replace(/\([0-9]*\)/g,"").*/ replace(/\(/g, "-").replace(/\)/g, "").toLowerCase(); //.toLowerCase().replace(/set_/g,"get_").replace(/\([0-9]\)/g,"");
										var valuevar = values[1].toLowerCase().replace(/[^\w\d]/g, "");
										console.log("111111111111111111111111111111111valueid:" + valueid + " 2222222222222222222222222222222222valuevar:" + valuevar);
										if(valueid != null && valueid != "" && values[1] != null) {
											var element = document.getElementById(valueid);
											if(element) {
												document.getElementById(valueid).innerHTML = values[1];
											}
										}
...
function calctpod(type) {
	...
	if(type == "heat") {
		console.log("type:" + type + " kwheat:" + kwheat + " tpidheat:" + tpidheat);
		heattpod.innerHTML = tpidheat - kwheat * tout;
	} else if(type == "cool") {
		console.log("type:" + type + " kwcool:" + kwcool + " tpidcool:" + tpidcool);
...
function updatelog() { //console.log("updatelog");
