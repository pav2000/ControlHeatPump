...
static void errorBlink(int n) {
  ...
  noInterrupts();
  GPBR->SYS_GPBR[0] |= n; 					    // pav2000  сохранить отладочную информацию
  //WDT_Restart(WDT);                            // pav2000
...
void vApplicationStackOverflowHook(TaskHandle_t pxTask, char *pcTaskName) {
	...
	for(uint8_t i = 0; i < 3; i++) {
		GPBR->SYS_GPBR[1 + i] = *((uint32_t *)pcTaskName + i);
	}
	errorBlink(3);
}
//------------------------------------------------------------------------------
...
/** pav2000 не верный номер задачи уже все пошло не так данные разрушены*/
void crash_data_Handler() {errorBlink(7);}
/*-----------------------------------------------------------*/
