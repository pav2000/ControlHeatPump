...
#include <Arduino.h>   //pav2000  нужно для записи текущего номера задачи для отладки
/* Standard includes. */
#include <stdlib.h>
#include <string.h>
...
void vTaskSwitchContext( void )
{
	...
	else
	{
		...
		#if ( configGENERATE_RUN_TIME_STATS == 1 )
                //pav2000 запомнить текущую задачу считаем что задач больше 32 не будет, если больше то уже проблемы
                if (pxCurrentTCB->uxTCBNumber<48) GPBR->SYS_GPBR[0] = pxCurrentTCB->uxTCBNumber<<8; // Запомнить текущую задачу сдвинув значение на 8 бит влево (место для номера ошибки)
                else  crash_data_Handler();   // pav2000 данные о задачах разрушены т.к. номер задачи бредовый
		#endif
...
void vTaskList( char * pcWriteBuffer )
{
	...
	const char tskREADY[] 		= "Ready";
	const char tskBLOCKED[] 	= "Blocked";
	const char tskSUSPENDED[]	= "Suspended";
	const char tskDELETED[]		= "Deleted";
	const char tskERROR[]		= "Error";
...
#endif /* ( ( configUSE_TRACE_FACILITY == 1 ) && ( configUSE_STATS_FORMATTING_FUNCTIONS > 0 ) ) */
#endif //configGENERATE_RUN_TIME_STATS_TASKLIST
/*----------------------------------------------------------*/

#ifndef configGENERATE_RUN_TIME_STATS_TASKLIST
...
#if ( configGENERATE_RUN_TIME_STATS == 1 )
// pav2000     Получить номер текущей задачи, нужно для генерации ошибки
inline int16_t getCurrentTaskNumber()
{
//strcpy(name, pxCurrentTCB->taskname);
return (int16_t)pxCurrentTCB->uxTCBNumber;
