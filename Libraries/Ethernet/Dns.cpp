...
int DNSClient::getHostByName(const char* aHostname, IPAddress& aResult,uint8_t sock) // pav2000
{
    int ret =0;
...
int16_t DNSClient::ProcessResponse(uint16_t aTimeout, IPAddress& aAddress)
{
 ...
 static   uint8_t header[DNS_HEADER_SIZE]; // Буфер для заголовка Enough space to reuse for the DNS header
    // Check that it's a response from the right server and the right port
    // Проверяем, что это ответ от нужного сервера и нужного порта
    if ( (iDNSServer != iUdp.remoteIP()) || 
        (iUdp.remotePort() != DNS_PORT) )
    {
        // It's not from who we expected
        // Это не от кого мы ожидали
        return INVALID_SERVER;
    }
    ...
    {
        return TRUNCATED;     // Размер маловат
    }
    iUdp.read(header, DNS_HEADER_SIZE);   // читаем заголовок пакета
