...
boolean HeatPump::switch3WAY(boolean b)
{
 ...
 relay3Way=b;                                    // запомнить состояние ВСЕГДА
   ...
   if((relay3Way!=b)&&(get_State()==pWORK_HP))   // Если было перекидывание 3-х ходового и ТН работает до обеспечить дополнительное время (20 сек) для прокачивания гликоля - т.к разные уставки по температуре подачи
       {
...
if ((abs(FEED-RET)>Prof.Heat.dt)&&(COMPRESSOR_IS_ON))   {set_Error(ERR_DTEMP_CON,(char*)__FUNCTION__);return pCOMP_NONE;}// Привышение разности температур кондесатора при включеноом компрессорае
#ifdef RTRV    
...
switch (Prof.Heat.Rule)   // в зависмости от алгоритма
         {
          case pHYSTERESIS:  // Гистерезис нагрев.
              Status.ret=pNone;                                                                                   // Сбросить состояние пида
               ...
               else if(FEED>Prof.Heat.tempIn)      {Status.ret=pHh1;   return pCOMP_OFF;}                            // Достигнута максимальная температура подачи ВЫКЛ
          ...
          case pPID:   // ПИД регулирует подачу, а целевай функция гистререзис
            ...
            if (GETBIT(Prof.Heat.flags,fTarget)) { target=Prof.Heat.Temp2; t1=RET;}  else { target=Prof.Heat.Temp1; t1=sTemp[TIN].get_Temp();} // вычислить темературы для сравнения Prof.Heat.Target 0-дом  1-обратка
             ...
             else if(FEED>Prof.Heat.tempIn) {Status.ret=pHp1; set_Error(ERR_PID_FEED,(char*)__FUNCTION__);return pCOMP_OFF;}        // Достижение максимальной температуры подачи - это ошибка ПИД не рабоатет
...
if ((abs(FEED-RET)>Prof.Cool.dt)&&(COMPRESSOR_IS_ON))   {set_Error(ERR_DTEMP_CON,(char*)__FUNCTION__);return pCOMP_NONE;}// Привышение разности температур кондесатора при включеноом компрессорае
#ifdef RTRV
...
switch (Prof.Cool.Rule)   // в зависмости от алгоритма
         {
          case pHYSTERESIS:  // Гистерезис охлаждение.
              Status.ret=pNone;                                                                                   // Сбросить состояние пида
               ...
               else if(FEED<Prof.Cool.tempIn)      {Status.ret=pCh1;   return pCOMP_OFF;}                            // Достигнута минимальная температура подачи ВЫКЛ
          ...
          case pPID:   // ПИД регулирует подачу, а целевай функция гистререзис
            ...
            if (GETBIT(Prof.Cool.flags,fTarget)) { target=Prof.Cool.Temp2; t1=RET;}  else { target=Prof.Cool.Temp1; t1=sTemp[TIN].get_Temp();} // вычислить темературы для сравнения Prof.Heat.Target 0-дом  1-обратка
             ...
             else if(FEED<Prof.Cool.tempIn) {Status.ret=pCp1; set_Error(ERR_PID_FEED,(char*)__FUNCTION__);return pCOMP_OFF;}         // Достижение минимальной температуры подачи - это ошибка ПИД не рабоатет
