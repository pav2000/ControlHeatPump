...
boolean HeatPump::set_optionHP(char *var, float x)   
{
   ...
   if(strcmp(var,option_NEXT_DIM)==0)         {if ((x>=1.0)&&(x<=100.0)) {Option.dim=x; updateNextion(); return true;} else return false;                                                       }else       // Якрость % NEXTION
...
int8_t HeatPump::StartResume(boolean start)
{
	...
	// 5. Конфигурируем ТН -----------------------------------------------------------------------
	if (get_State()!=pSTARTING_HP) return error;            // Могли нажать кнопку стоп, выход из процесса запуска
	else  configHP(Status.modWork);                         // Конфигурируем 3 и 4-х клапаны и включаем насосы ПАУЗА после включения насосов
