...
int8_t HeatPump::load_motoHour()          
{
 ...
 /* 
   // Код можно убрать в июле 2020 когда все прейдут 
   // Восстановление значений счетчиков, требуется при переходе с 1.043 на 1.055 версию, ПРИМЕРНО!
   // в старой версии записываем значения и потом здесь вводим, после обновления, этот фрагмент кода удаляем 
   // дата в юникс формате https://www.cy-pr.com/tools/time/
...
void  HeatPump::updateChart()
{
	...
	if(ChartTPEVA.get_present())     ChartTPEVA.addPoint(PressToTemp(PEVA));
//	if (sADC[PCON].get_present())    // Если датчик высокого давления есть считаем честно
//    	{ if(ChartTPCON.get_present()) ChartTPCON.addPoint(PressToTemp(PCON));}
//	else 
