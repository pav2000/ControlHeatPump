...
// 2. При переключениеи с ГВС сделано временное отключение (DELAY_BOILER_OFF) части защит, на время остывания ТН.
...
// 13. Переделан механизм работы с датчиками температуры. Теперь разрешено три вида шин OneWire и одноврменно может присутвовать три вида шин в контроллере.
// 14. Добавлено расписание работы (пока не до конца)
// 15. Переделан вид расписания бойлера на веб морде
// 16. Добавлена страница на вебморду для вывода информации по задачам операционной системы
...
// 19. В связи с работай над расписаниями, удален функционал тарифов день/ночь
     ...
     if ((HP.get_State()==pOFF_HP)||(HP.get_State()==pSTOPING_HP))                                     // Если  насос не работает или идет останов насоса то остановить задачу Обновления ТН (Вообще то это лишнее, надо убрать)
     {
      ...
      // 3. Управление циркуляционным насосом ГВС
       ...
       #endif
        {
           ...
           {
            if ((HP.dRelay[RCOMP].get_Relay()||HP.dFC.isfOnOff())&&(HP.get_relay3Way())) { HP.dRelay[RPUMPB].set_ON(); continue;} // идет нагрев ГВС включаем насос ГВС ВСЕГДА - улучшаем перемешивание
