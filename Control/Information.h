...
class statChart                                         // организован кольцевой буфер
{
 public: 
  ...
  int16_t get_Point(uint16_t x);                        // получить точку нумерация 0-самая старая CHART_POINT - самая новая, (работает кольцевой буфер)
  ...
  char *get_PointsStr(uint16_t m, char *b);             // получить строку в которой перечислены все точки в строковом виде через ";" при этом значения делятся на m
  char *get_boolPointsStr(uint16_t mask,  char *b);     // БИНАРНЫЕ данные: получить строку в которой перечислены все точки через ";" при этом на значения накладывается mask (0 или 1)
  
  inline boolean get_present() {return present;}        // Наличие датчика в текущей конфигурации
  uint16_t get_num()  {return num;}                     // Получить число накопленных точек
 private:
  int8_t err;                                            // Ошибка
 // int16_t data[CHART_POINT];                           // массив для накопления данных
...
#define fHP_ON    0                // флаг Включения ТН
...
struct type_settingHP
{
 ...
 int16_t Kd;                          // Дифференциальная составляющая ПИД ТН
 // Защиты
 int16_t tempIn;                      // Tемпература подачи (минимальная или минимальная)
 int16_t tempOut;                     // Tемпература обратки (минимальная или минимальная)
 uint16_t pause;                      // Минимальное время простоя компрессора в секундах
