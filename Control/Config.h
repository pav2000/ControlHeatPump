...
//                                        "SERRFC"           // Сигнал ошибка инвертора
 ...
 //                                         ,PIN_SENSOR_FLOWPCON     // Датчик протока по предконденсатору
      ...
      // Описание датчиков
    const char *noteFrequency[FNUMBER] = { "Датчик потока по испарителю (кубы/час)",          // Датчик потока по испарителю
                                           "Датчик потока по кондесатору (кубы/час)"        //  Датчик потока по кондесатору
 ...
 //                                         ,"FLOWPCON"       // Датчик протока по предконденсатору
   ...
   // Исполнительные устройства (реле и сухие контакты) ВНИМАТЕЛЬНО ПРОВЕРЯЕМ СООТВЕТСВИЕ ВСЕХ МАССИВОВ!!! ------------------------------------------------------------------
    #define RNUMBER                    7        // Число исполнительных устройств (всех)
     // устройства DC 24V
    #define PIN_DEVICE_RCOMP           27 //[R_9(X1)3.3V] через R=680->4N33: 24V(6) -> DI1(8). Реле включения компрессора. P5.1=0, P5.7=1
