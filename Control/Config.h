...
// Описание реле
  ...
  //   #define EEV_DEF                     // Наличие ЭРВ в конфигурации
          ...
          #define delayOff     10             // Задержка закрытия EEV после выключения насосов (сек). Время от команды стоп компрессора до закрытия ЭРВ = DELAY_OFF_PUMP+delayOff
       #endif   
      ...
      #define startFreq       (60*100)       // Стартовая частота частота инвертора (см компрессор) в 0.01 Гц
      #define startFreq_BOILER (50*100)      // Стартовая частота частота инвертора (см компрессор) в 0.01 Гц ГВС
      
      #define minFreq         (40*100)       // Минимальная  частота инвертора (см компрессор) в 0.01 Гц
      #define minFreqCool    (60*100)       // Минимальная  частота инвертора при охлаждении в 0.01 Гц
      #define minFreqBoiler  (45*100)       // Минимальная  частота инвертора при нагреве ГВС в 0.01 Гц
      #define minFreqUser    (35*100)       // Минимальная  частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 Гц
       
      #define maxFreq         (110*100)      // Максимальная частота инвертора (см компрессор) в 0.01 Гц
      #define maxFreqCool    (100*100)      // Максимальная частота инвертора в режиме охлаждения  в 0.01 Гц
      #define maxFreqBoiler  (60*100)       // Максимальная частота инвертора в режиме ГВС в 0.01 Гц поглощение бойлера обычно меньше чем СО
       #define maxFreqUser   (190*100)      // Максимальная частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 Гц
                
      #define stepFreq        (1*100)        // Шаг уменьшения частоты инвертора при достижении максимальной температуры, мощности и тока (см компрессор) в 0.01 Гц
      #define stepFreqBoiler (2*100)        // Шаг уменьшения частоты инвертора при достижении максимальной температуры, мощности и тока ГВС в 0.01 Гц
      
      #define dtTemp        (1*100)        // Привышение температуры от уставок (подача) при которой срабатыват защита (уменьшается частота) в сотых градуса
      #define dtTemp_BOILER (2*100)        // Привышение температуры от уставок (подача) при которой срабатыват защита ГВС в сотых градуса
    ...
    #define DELAY_BOILER_SW   60             // Пауза (сек) после переключение ГВС - выравниваем температуру в контуре отопления/ГВС что бы сразу защиты не сработали
...
//###################################################################################################################################################################################################################
      ...
      #define EEV_DEF                     // Наличие ЭРВ в конфигурации
          ...
          #define preStartPos        100            // ПУСКОВАЯ позиция ЭРВ (ТО что при старте компрессора ПРИ РАСКРУТКЕ)
          #define StartPos         100            // СТАРТОВАЯ позиция ЭРВ после раскрутки компрессора т.е. ПОЗИЦИЯ С КОТОРОЙ НАЧИНАЕТСЯ РАБОТА проходит DelayStartPos сек
          #define minSteps     2              // Минимальное число шагов открытия ЭРВ
          #define EEV_HOLD_MOTOR    false           // Режим "удержания" шагового двигателя ЭРВ
          #define EEV_PHASE         PHASE_8s       // ЗАДАННАЯ (можно менять) последовательность (шагов) при движении ЭРВ
          // ЭРВ Времена и задержки
          #define delayOnPid  40             // Задержка включения EEV после включения компрессора (сек).  Точнее после выхода на рабочую позицию Общее время =delayOnPid+DelayStartPos
          #define delayOn     1              // Задержка между открытием (для старта) ЭРВ и включением компрессора, для выравнивания давлений (сек). Если ЭРВ закрывлось при остановке
          #define DelayStartPos   1             // Время после старта компрессора когда EEV выходит на стартовую позицию - облегчение пуска вначале ЭРВ
          #define delayOff     1             // Задержка закрытия EEV после выключения насосов (сек). Время от команды стоп компрессора до закрытия ЭРВ = DELAY_OFF_PUMP+delayOff
       #endif   
//###################################################################################################################################################################################################################
      // Частотный преобразователь ТОЛЬКО ОДНА ШТУКА ВСЕГДА (не массив) --------------------------------------------------------
      ...
      #define PidFreqStep       (1*100)        // Максимальный шаг (на увеличение) изменения частоты при ПИД регулировании в 0.01 Гц Необходимо что бы ЭРВ успевал
      #define PidStop           95            // Проценты от уровня защит (мощность, ток, давление, темпеартура) при которой происходит блокировка роста частоты пидом

      #define dtCompTemp       (5*100)       // Защита по температуре компрессора - сколько градусов не доходит до максимальной (TCOMP) и при этом происходит уменьшение частоты
      #define FC_DT_CON_PRESS       300           // Защита по давлению компрессора - сколько сотых бара не доходит до максимальной (PCON) и при этом происходит уменьшение частоты

      #define startFreq           (35*100)       // Стартовая частота частота инвертора (см компрессор) в 0.01 Гц
      #define startFreq_BOILER    (35*100)       // Стартовая частота частота инвертора (см компрессор) в 0.01 Гц ГВС
      
      #define minFreq             (35*100)       // Минимальная  частота инвертора (см компрессор) в 0.01 Гц
      #define minFreqCool        (35*100)       // Минимальная  частота инвертора при охлаждении в 0.01 Гц
      #define minFreqBoiler      (35*100)       // Минимальная  частота инвертора при нагреве ГВС в 0.01 Гц
      #define minFreqUser        (35*100)       // Минимальная  частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 Гц
      
      #define maxFreq             (180*100)      // Максимальная частота инвертора (см компрессор) в 0.01 Гц
      #define maxFreqCool        (180*100)      // Максимальная частота инвертора в режиме охлаждения  в 0.01 Гц
      #define maxFreqBoiler      (180*100)      // Максимальная частота инвертора в режиме ГВС в 0.01 Гц поглощение бойлера обычно меньше чем СО
      #define maxFreqUser        (180*100)      // Максимальная частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 Гц
                
      #define stepFreq            (3*100)       // Шаг уменьшения частоты инвертора при достижении максимальной температуры, мощности и тока (см компрессор) в 0.01 Гц
      #define stepFreqBoiler     (1*100)       // Шаг уменьшения частоты инвертора при достижении максимальной температуры, мощности и тока ГВС в 0.01 Гц
      
      #define dtTemp            (1*100)        // Привышение температуры от уставок (подача) при которой срабатыват защита (уменьшается частота) в сотых градуса
      #define dtTemp_BOILER     (1*100)        // Привышение температуры от уставок (подача) при которой срабатыват защита ГВС в сотых градуса
       
      #define FC_MAX_POWER          (2.0*10)       // Максимальная мощность инвертора (см компрессор) в 0.1 кВт
      #define FC_MAX_POWER_BOILER   (1.9*10)       // Максимальная мощность инвертора в режиме ГВС (см компрессор) в 0.1 кВт
  ...
  //                                   "REVI",           // Соленойд для EVI. (испаритель ниже +3гр и конденсатор выше +40гр)
    ...
    // Могут быть задействованы разные датчики!
     ...
     const int16_t TESTTEMP[TNUMBER]={1000,2000,300,400,1000,3000,700,500,500,500,500,500,2000,2000}; 
      ...
      #define DEF_PidFreqStep    (2*100)        // Максимальный шаг (на увеличение) изменения частоты при ПИД регулировании в 0.01 Гц Необходимо что бы ЭРВ успевал
      #define DEF_PidStop          95          // Проценты от уровня защит (мощность, ток, давление, темпеартура) при которой происходит блокировка роста частоты пидом

      #define DEF_dtCompTemp   (5*100)        // Защита по температуре компрессора - сколько градусов не доходит до максимальной (TCOMP) и при этом происходит уменьшение частоты
      #define DEF_FC_DT_CON_PRESS   50             // Защита по давлению компрессора - сколько сотых бара не доходит до максимальной (PCON) и при этом происходит уменьшение частоты

      #define DEF_startFreq       (60*100)       // Стартовая частота частота инвертора (см компрессор) в 0.01 Гц
      #define DEF_startFreq_BOILER (50*100)      // Стартовая частота частота инвертора (см компрессор) в 0.01 Гц ГВС
      
      #define DEF_minFreq         (40*100)       // Минимальная  частота инвертора (см компрессор) в 0.01 Гц
      #define DEF_minFreqCool    (60*100)       // Минимальная  частота инвертора при охлаждении в 0.01 Гц
      #define DEF_minFreqBoiler  (45*100)       // Минимальная  частота инвертора при нагреве ГВС в 0.01 Гц
      #define DEF_minFreqUser    (35*100)       // Минимальная  частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 Гц
      
      #define DEF_maxFreq         (110*100)      // Максимальная частота инвертора (см компрессор) в 0.01 Гц
      #define DEF_maxFreqCool    (100*100)      // Максимальная частота инвертора в режиме охлаждения  в 0.01 Гц
      #define DEF_maxFreqBoiler  (60*100)       // Максимальная частота инвертора в режиме ГВС в 0.01 Гц поглощение бойлера обычно меньше чем СО
      #define DEF_maxFreqUser    (190*100)      // Максимальная частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 Гц
                
      #define DEF_stepFreq        (1*100)        // Шаг уменьшения частоты инвертора при достижении максимальной температуры, мощности и тока (см компрессор) в 0.01 Гц
      #define DEF_stepFreqBoiler (2*100)        // Шаг уменьшения частоты инвертора при достижении максимальной температуры, мощности и тока ГВС в 0.01 Гц
      
      #define DEF_dtTemp        (1*100)        // Привышение температуры от уставок (подача) при которой срабатыват защита (уменьшается частота) в сотых градуса
      #define DEF_dtTemp_BOILER (2*100)        // Привышение температуры от уставок (подача) при которой срабатыват защита ГВС в сотых градуса
...
// Описание реле
     ...
     */
     ...
     // Коэффициент преобразования отсчеты АЦП-давление
      ...
      #define EEV_DEF                     // Наличие ЭРВ в конфигурации
	    ...
	    #define DEFAULT_DELAY_OFF       10            // Задержка закрытия EEV после выключения насосов (сек). Время от команды стоп компрессора до закрытия ЭРВ = DELAY_OFF_PUMP+delayOff
	  #endif
 ...
 //     #define FC_ANALOG_CONTROL                      // Использование аналогового управления частотником, Для использования 485 закоментировать эту строку
      ...
      #endif  
      #define USE_RCOMP                           // Использовать для пуска/остановки инвертора отдельный выход RCOMP (команда по модбасу не посылается)

    // Константы частотного преобразователя для конкртеной  реализации  (из программы не меняются)  
  	#define FC_DT_CON_PRESS      50                 // Защита по давлению компрессора - сколько сотых бара не доходит до максимальной (PCON) и при этом происходит уменьшение частоты
  	#define FC_MAX_POWER         (1.8*10)           // Максимальная мощность инвертора (см компрессор) в 0.1% от номинала
	#define FC_MAX_POWER_BOILER  (1.3*10)           // Максимальная мощность инвертора в режиме ГВС (см компрессор) в 0.1% от номинала
	#define FC_MAX_CURRENT        (10.0*100)        // Максимальный ток инвертора (см компрессор) в 0.01 А
	#define FC_MAX_CURRENT_BOILER (11.0*100)        // Максимальный ток инвертора для ГВС в 0.01 А
 													
 	// Значения по умолчанию (можно менять из морды)												
    #define DEF_FC_UPTIME            (10*1000)      // Время обновления алгоритма пид регулятора (мсек) Основной цикл управления
...
//	#define TEST_BOARD 				// Тестовая плата!
	...
	#define SDM_MAX_POWER       4500.0         // Максимальная мощность потребления (ватт) - выключение ТН
...
//                                        "SERRFC"           // Сигнал ошибка инвертора
 ...
 //                                         ,"FLOWPCON"       // Датчик протока по предконденсатору
    ...
    const float TRANSFLOW[FNUMBER]   = {100,  100};       // Коэффициент преобразования частота - поток
    const uint16_t TESTFLOW[FNUMBER] = {2000, 1500};      // Значения датчиков при тестировании  опция TEST
    const uint16_t MINFLOW[FNUMBER] =  {1000, 500};       // МИНИМАЛЬНЫЕ Значения датчиков
...
//                                     Пины DUE 68, 69 не подключены к плате!
    ...
    // Имена индексов ВАЖЕН ПОРЯДОК!
    #define RCOMP              0          // Реле включения компрессора (через пускатель)
    #define RPUMPO             1          // Реле включения насоса выхордного контура  (отопление и/или ГВС)
...
// Описание реле
  ...
  #define OVERRIDE_TNUMBERS
    ...
    #define TCONOUTG    7    // Температура на выходе конденсатора (по гликолю)  - 28E6C1250600004A
    #define TEVAOUT     8    // Температура на выходе испарителя (по фреону)     - 285960280600000A
    #define TCONOUT     9    // Температура на выходе конденсатора (по фреону)   - 28FF509C55140355
...
//        "Температура на выходе теплоаккумулятора",
  ...
  #define TIME_READ_SENSOR  1250 //4000  // Период опроса датчиков (мсек), к нему добавляется время DELAY_DS1820(750мсек) и 2мсек * TNUMBER
  ...
  */
  ...
  #define EEV_DEF                     // Наличие ЭРВ в конфигурации
    ...
    #define DEFAULT_PRE_START_POS  150            // ПУСКОВАЯ позиция ЭРВ (ТО что при старте компрессора ПРИ РАСКРУТКЕ)
    #define DEFAULT_START_POS      100            // СТАРТОВАЯ позиция ЭРВ после раскрутки компрессора т.е. ПОЗИЦИЯ С КОТОРОЙ НАЧИНАЕТСЯ РАБОТА проходит DelayStartPos сек
    #define DEFAULT_MIN_STEP       10             // Минимальное число шагов открытия ЭРВ
    #define DEFAULT_HOLD_MOTOR     false          // Режим "удержания" шагового двигателя ЭРВ
    // ЭРВ Времена и задержки
    #define DEFAULT_DELAY_ON_PID    5             // Задержка включения EEV после включения компрессора (сек).  Точнее после выхода на рабочую позицию Общее время =delayOnPid+DelayStartPos
    ...
    #define DEFAULT_DELAY_OFF       3             // Задержка закрытия EEV после выключения насосов (сек). Время от команды стоп компрессора до закрытия ЭРВ = DELAY_OFF_PUMP+delayOff
  #endif
 ...
 //  #define ANALOG_CONTROL                      // Использование аналогового управления частотником, Для использования 485 закоментировать эту строку
  	// Регистры Vacon modbus: 101(мин.частота)=4000, 102(макс.частота)=7000, 117=3, 172=0
    // Константы частотного преобразователя для конкртеной  реализации  (из программы не меняются)  
  	#define FC_DT_CON_PRESS      50             // Защита по давлению компрессора - сколько сотых бара не доходит до максимальной (PCON) и при этом происходит уменьшение частоты
	...
	#define FC_MAX_CURRENT        (10.0*100)     // Максимальный ток инвертора (см компрессор) в 0.01 А
	#define FC_MAX_CURRENT_BOILER (10.0*100)     // Максимальный ток инвертора для ГВС в 0.01 А
    #define FC_ACCEL_TIME         (30*100)       // Время разгона (см компрессор) в 0.01 сек
    #define FC_DEACCEL_TIME       (20*100)       // Время торможения (см компрессор) в 0.01 сек  надо ставить 0 что бы небыло генерации при торможении
	...
	#define DEF_FC_PID_STOP          95             // Проценты от уровня защит (мощность, ток, давление, темпеартура) при которой происходит блокировка роста частоты пидом
	#define DEF_FC_DT_COMP_TEMP      (5*100)        // Защита по температуре компрессора - сколько градусов не доходит до максимальной (TCOMP) и при этом происходит уменьшение частоты
	#define DEF_FC_START_FREQ        (50*100)       // Стартовая частота инвертора (см компрессор) в 0.01 %
	#define DEF_FC_START_FREQ_BOILER (40*100)       // Стартовая частота инвертора (см компрессор) в 0.01 % ГВС
	#define DEF_FC_MIN_FREQ          (10*100)       // Минимальная  частота инвертора (см компрессор) в 0.01 %
	#define DEF_FC_MIN_FREQ_COOL     (10*100)       // Минимальная  частота инвертора при охлаждении в 0.01 %
	#define DEF_FC_MIN_FREQ_BOILER   (10*100)       // Минимальная  частота инвертора при нагреве ГВС в 0.01 %
	#define DEF_FC_MIN_FREQ_USER     (25*100)       // Минимальная  частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 %
	#define DEF_FC_MAX_FREQ          (100*100)      // Максимальная частота инвертора (см компрессор) в 0.01 %
	#define DEF_FC_MAX_FREQ_COOL     (100*100)      // Максимальная частота инвертора в режиме охлаждения  в 0.01 %
	#define DEF_FC_MAX_FREQ_BOILER   (80*100)       // Максимальная частота инвертора в режиме ГВС в 0.01 Гц поглощение бойлера обычно меньше чем СО
	#define DEF_FC_MAX_FREQ_USER     (190*100)      // Максимальная частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 %
