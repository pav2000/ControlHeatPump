...
// Описание реле
 ...
 // #endif
  ...
  const uint16_t MAXPRESS[ANUMBER]  = {  700,  2000,  250,  280};	// Максимальные значения давления, в сотых бар
  ...
  #define FC_VACON        // Частотник Vacon
    ...
    #define DEF_DELAY_DEFROST_ON 		120            // ДЛЯ ВОЗДУШНОГО ТН Задержка после срабатывания датчика перед включением разморозки (секунды)
    #define DEF_DELAY_DEFROST_OFF		120            // ДЛЯ ВОЗДУШНОГО ТН Задержка перед выключением разморозки (секунды)
    #define DEF_DELAY_TRV        		20             // Задержка между переключением 4-х ходового клапана и включением компрессора, для выравнивания давлений (сек). Если включены эти опции (переключение тепло-холод)
