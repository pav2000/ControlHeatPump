...
// -----------------------------------------------------------------------------------------------------------------------------------
 ...
 //   #define DEFROST                                                           // нужна разморозка
   ...
   //    #define ONEWIRE_DONT_CHG_RES                                         // Не менять разрешение датчиков
		...
		#ifdef  I2C_EEPROM_64KB                    // В зависимости от типа чипа
			#define I2C_ADR_EEPROM    0x50         // Адрес чипа eeprom на шине I2C
			#define I2C_SIZE_EEPROM   1024    	   // Объем чипа в килобитах
...
// Описание реле
     ...
     */
     ...
     // Коэффициент преобразования отсчеты АЦП-давление
      ...
      #define EEV_DEF                     // Наличие ЭРВ в конфигурации
          ...
          #define EEV_STEPS         490            // Число шагов ЭРВ 480
          #define EEV_PSTART        220            // ПУСКОВАЯ позиция ЭРВ (ТО что при старте компрессора ПРИ РАСКРУТКЕ)
          #define EEV_START         130            // СТАРТОВАЯ позиция ЭРВ после раскрутки компрессора т.е. ПОЗИЦИЯ С КОТОРОЙ НАЧИНАЕТСЯ РАБОТА проходит DELAY_START_POS сек
          #define EEV_MIN_STEPS     2              // Минимальное число шагов открытия ЭРВ
          #define EEV_HOLD_MOTOR    false          // Режим "удержания" шагового двигателя ЭРВ
          #define EEV_PHASE         PHASE_8s       // ЗАДАННАЯ (можно менять) последовательность (шагов) при движении ЭРВ
          // ЭРВ Времена и задержки
          #define DELAY_ON_PID_EEV  90             // Задержка включения EEV после включения компрессора (сек).  Точнее после выхода на рабочую позицию Общее время =DELAY_ON_PID_EEV+DELAY_START_POS
          #define DELAY_ON3_EEV     5              // Задержка между открытием (для старта) ЭРВ и включением компрессора, для выравнивания давлений (сек). Если ЭРВ закрывлось при остановке
          #define DELAY_START_POS   30             // Время после старта компрессора когда EEV выходит на стартовую позицию - облегчение пуска вначале ЭРВ (время разгона)
          #define DELAY_OFF_EEV     10             // Задержка закрытия EEV после выключения насосов (сек). Время от команды стоп компрессора до закрытия ЭРВ = DELAY_OFF_PUMP+DELAY_OFF_EEV
       #endif       
 ...
 //     #define FC_ANALOG_CONTROL                      // Использование аналогового управления частотником, Для использования 485 закоментировать эту строку
      ...
      #define FC_START_FREQ        (55*100)       // Стартовая частота частота инвертора отопление/охлаждение (см компрессор) в 0.01 Гц
      #define FC_START_FREQ_BOILER (45*100)      // Стартовая частота частота инвертора ГВС (см компрессор) в 0.01 Гц ГВС
      ...
      #define FC_MAX_FREQ_COOL    (100*100)      // Максимальная частота инвертора в режиме охлаждения  в 0.01 Гц
      #define FC_MAX_FREQ_BOILER  (50*100)       // Максимальная частота инвертора в режиме ГВС в 0.01 Гц поглощение бойлера обычно меньше чем СО
      ...
      #define FC_MAX_POWER_BOILER (1.3*10)       // Максимальная мощность инвертора в режиме ГВС (см компрессор) в 0.1 кВт
...
//        "Температура на выходе теплоаккумулятора",
  ...
  */
  ...
  #define EEV_DEF                     // Наличие ЭРВ в конфигурации
    ...
    //#define EEV_INVERT                      // Признак инвертирования движения ЭРВ  (меняем если крутит в обратную сторону)
    //#define EEV_INT_PID                     // использование ПИДА в целочисленной арифметике
    #define DEFAULT_EEV_KP_ERR         100.0  // Ошибка (в сотых градуса) при которой происходит уменьшение пропорциональной составляющей ПИД ЭРВ
    #define DEFAULT_EEV_SPEED          40     // Скорость шагового двигателя ЭРВ (импульсы в сек.)
    #define DEFAULT_EEV_PSTART         150    // ПУСКОВАЯ позиция ЭРВ (ТО что при старте компрессора ПРИ РАСКРУТКЕ)
    #define DEFAULT_EEV_START          100    // СТАРТОВАЯ позиция ЭРВ после раскрутки компрессора т.е. ПОЗИЦИЯ С КОТОРОЙ НАЧИНАЕТСЯ РАБОТА проходит DELAY_START_POS сек
    #define DEFAULT_EEV_MIN_STEPS      10     // Минимальное число шагов открытия ЭРВ
    #define DEFAULT_EEV_HOLD_MOTOR     false  // Режим "удержания" шагового двигателя ЭРВ
    // ЭРВ Времена и задержки
    #define DEFAULT_DELAY_ON_PID_EEV   5      // Задержка включения EEV после включения компрессора (сек).  Точнее после выхода на рабочую позицию Общее время =DELAY_ON_PID_EEV+DELAY_START_POS
    #define DEFAULT_DELAY_ON3_EEV      3      // Задержка между открытием (для старта) ЭРВ и включением компрессора, для выравнивания давлений (сек). Если ЭРВ закрывлось при остановке
    ...
    #define DEFAULT_DELAY_OFF_EEV      3      // Задержка закрытия EEV после выключения насосов (сек). Время от команды стоп компрессора до закрытия ЭРВ = DELAY_OFF_PUMP+DELAY_OFF_EEV
  #endif
 ...
 //  #define ANALOG_CONTROL                      // Использование аналогового управления частотником, Для использования 485 закоментировать эту строку
      // Константы частотного преобразователя для конкртеной  реализации
	#define DEF_FC_USE_RCOMP                     // Использовать для разрешения работы инвертора отдельный выход RCOMP
	...
	#define DEF_FC_START_FREQ        (50*100)       // Стартовая скорость инвертора (см компрессор) в 0.01 %
	#define DEF_FC_START_FREQ_BOILER (40*100)      // Стартовая скорость инвертора (см компрессор) в 0.01 % ГВС
	...
	#define DEF_FC_MIN_FREQ_USER     (25*100)       // Минимальная  скорость инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 %
	#define DEF_FC_MAX_FREQ          (100*100)      // Максимальная скорость инвертора (см компрессор) в 0.01 %
	#define DEF_FC_MAX_FREQ_COOL     (100*100)      // Максимальная скорость инвертора в режиме охлаждения  в 0.01 %
	#define DEF_FC_MAX_FREQ_BOILER   (80*100)       // Максимальная скорость инвертора в режиме ГВС в 0.01 Гц поглощение бойлера обычно меньше чем СО
	#define DEF_FC_MAX_FREQ_USER     (190*100)      // Максимальная скорость инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 %
