...
//###################################################################################################################################################################################################################
      ...
      #define FC_STEP_FREQ_BOILER     (1*100)       // Шаг уменьшения частоты инвертора при достижении максимальной температуры, мощности и тока ГВС в 0.01 Гц
...
// Описание реле
 ...
 //     #define FC_ANALOG_CONTROL                      // Использование аналогового управления частотником, Для использования 485 закоментировать эту строку
      ...
      #define DEF_FC_MAX_POWER        (1.8*10)       // Максимальная мощность инвертора (см компрессор) в 0.1 кВт
      #define DEF_FC_MAX_POWER_BOILER (1.3*10)       // Максимальная мощность инвертора в режиме ГВС (см компрессор) в 0.1 кВт
    
      #define DEF_FC_MAX_CURRENT      (10.0*100)     // Максимальный ток инвертора (см компрессор) в 0.01 А
      #define DEF_FC_MAX_CURRENT_BOILER (11.0*100)   // Максимальный ток инвертора для ГВС в 0.01 А
   
      //#define FC_FULL_INIT                     // Полная инициализация инвертора при страте контроллера (записываются регистры нужные значеня) МОЖНО закоментировать
      // Для FC_FULL_INIT что прописывается в ПЧ
      #define DEF_FC_BASE_FREQ        (120*100)       // Основная частота инвертора (см компрессор) в 0.01 Гц
      #define DEF_FC_ACCEL_TIME     (60*100)        // Время разгона (см компрессор) в 0.01 сек
      #define DEF_FC_DEACCEL_TIME   (90*100)        // Время торможения (см компрессор) в 0.01 сек  надо ставить 0 что бы небыло генерации при торможении
...
//        "Температура на выходе теплоаккумулятора",
 ...
 //  #define ANALOG_CONTROL                      // Использование аналогового управления частотником, Для использования 485 закоментировать эту строку
	...
	#define DEF_FC_DT_TEMP           (1*100)        // Привышение температуры от уставок (подача) при которой срабатыват защита (уменьшается частота) в сотых градуса
	#define DEF_FC_DT_TEMP_BOILER    (2*100)        // Привышение температуры от уставок (подача) при которой срабатыват защита ГВС в сотых градуса
	#define DEF_FC_MAX_POWER         (100*10)         // Максимальная мощность инвертора (см компрессор) в 0.1% от номинала
	#define DEF_FC_MAX_POWER_BOILER  (80*10)          // Максимальная мощность инвертора в режиме ГВС (см компрессор) в 0.1% от номинала
	#define DEF_FC_MAX_CURRENT        (10.0*100)     // Максимальный ток инвертора (см компрессор) в 0.01 А
	#define DEF_FC_MAX_CURRENT_BOILER (10.0*100)     // Максимальный ток инвертора для ГВС в 0.01 А
	#define DEF_FC_ACCEL_TIME        (300*100)        // Время разгона (см компрессор) в 0.01 сек
	#define DEF_FC_DEACCEL_TIME      (300*100)        // Время торможения (см компрессор) в 0.01 сек  надо ставить 0 что бы небыло генерации при торможении
     // Временные задержки
