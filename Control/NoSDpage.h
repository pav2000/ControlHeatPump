...
"                 if(values[0] == \"get_presentRelay(RHEAT)\" && values[1] == 0) {element=document.getElementById(\"get_ohp-add_heat\");if(element) {element.disabled = true;}}\r\n"
"                 if(values[0] == \"get_presentRelay(REVI)\" && values[1] == 0) {element=document.getElementById(\"get_ohp-temp_evi\");if(element) {element.disabled = true;} element=document.getElementById(\"get_ohp-temp_evi2\");if(element) {element.disabled = true;}}\r\n"
...
"   <dt>Перегрев:</dt><dd id=\"get_parameev-overheat\">-</dd>\r\n"
"   <dd style=\"width:98%;\">\r\n"
"   <span id=\"get_error\" style=\"font-size:1em;line-height:1.3em\" ></span><br/>\r\n"
"<span style=\"font-size:0.8em;font-style:italic;line-height:1em\">SD карта не работает, показана упрощеная версия интерфейса. Для устраниния ошибки попробуйте на выключеном контроллере переставить карту, и выполнить сброс  контроллера. <br>"
"Расположение файлов веб-сервера на SPI Flash <input type='checkbox' id='get_ohp-wspif' onChange='setParam(\"get_oHP(WSPIF)\");'><br>"
"<input style=\"margin-top:15px;\" type=\"submit\" value=\"Сброс контроллера\"  onclick='loadParam(\"RESET\")'><a href=\"/journal.txt\" download=\"\"><button>Загрузить журнал</button></a></span>"
"   </dd> \r\n"
" </dl> \r\n"
"</div> \r\n"
"</body>\r\n"
