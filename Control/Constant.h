...
// 0х0080 - I2C_SETTING_EEPROM хранение настроек ТН  максимальный размер 0х980 (2432) байт. Сейчас используется 1230 байт
// 0x0A00 - I2C_PROFILE_EEPROM хранение профилей, максимальный размер 0x1000 (4096) байт. Число профилей 10 шт. Размер профиля сейчас 315 байт (3150)
// 0x1A00 - I2C_SCHEDULER_EEPROM хранения расписаний, максимум 0x5FE (1534) байт (сейчас 377 байта)
...
const char *prof_ID_PROFILE     = {"ID_PROFILE"};         // номер профиля, нумерация c 1
...
const char *option_DELAY_ON_PUMP      = {"DONP"};      			// Задержка включения компрессора после включения насосов (сек).
const char *option_DELAY_OFF_PUMP     = {"DOFP"};			     // Задержка выключения насосов после выключения компрессора (сек).
const char *option_DELAY_START_RES    = {"DSRES"};			    // Задержка включения ТН после внезапного сброса контроллера (сек.)
const char *option_DELAY_REPEAD_START = {"DRST"}; 				// Задержка перед повторным включениме ТН при ошибке (попытки пуска) секунды
const char *option_DELAY_DEFROST_ON   = {"DDFON"};   			// ДЛЯ ВОЗДУШНОГО ТН Задержка после срабатывания датчика перед включением разморозки (секунды)
const char *option_DELAY_DEFROST_OFF  = {"DDFOF"}; 				 // ДЛЯ ВОЗДУШНОГО ТН Задержка перед выключением разморозки (секунды)
const char *option_DELAY_TRV          = {"DRV"}; 		         // Задержка между переключением 4-х ходового клапана и включением компрессора, для выравнивания давлений (сек). Если включены эти опции (переключение тепло-холод)
const char *option_DELAY_BOILER_SW    = {"DBSW"};			    // Пауза (сек) после переключение ГВС - выравниваем температуру в контуре отопления/ГВС что бы сразу защиты не сработали
const char *option_DELAY_BOILER_OFF   = {"DBOF"};   // Время (сек) на сколько блокируются защиты при переходе с ГВС на отопление и охлаждение слишком горяче после ГВС
...
const char *noteError[] = {"Ok",                                                  //  0
                           ...
                           "Ошибка конфигурации температурного датчика (привязка к отсутвующей шине)",  		//-85
...
//  Перечисляемый тип - Типы ответов пост запросов
enum TYPE_RET_POST
{
