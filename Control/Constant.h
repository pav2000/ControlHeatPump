...
#define VERSION			"1.033"				// Версия прошивки
...
#define NTP_SERVER        "ntp2.stratum2.ru"// NTP сервер для синхронизации времени по ДЕФОЛТУ (готовимся к отключению буржуинского инета)
...
// ------------------- MQTT ----------------------------------
#define MQTT_WEB_TASK      0             // буфер какого потока использует MQTT (должно совпадать с потоком в котором проводится отсылка MQTT) обычно 0
...
#define ERR_MAX_VOLTAGE     -75        // Слишком большое напряжение сети (данные SDM120)
#define ERR_MAX_POWER       -76        // Слишком большая портебляемая мощность (данные SDM120)
