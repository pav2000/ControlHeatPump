...
#define VERSION         "0.952 beta"        // Версия прошивки
#ifndef UART_SPEED
...
#define SALLMONELA_DAY    2              // День когда включается алгоритм обеззараживания воды (Понедельник 1 воскресенье 7)
#define SALLMONELA_HOUR   2              // Час когда включается алгоритм обеззараживания воды
#define SALLMONELA_TEMP   (70*100)       // Температура которая поддерживается для обеззараживания (сотые градуса)
#define SALLMONELA_END    (40*60)        // Максимальная продолжительность цикла (сек), что бы цикл не длился бесконечно при не возможности достижения SALLMONELA_TEMP
...
const char *fc_cCURRENT          = {"cCORRENT"};          // Текущий ток (чтение)
const char *fc_AUTO              = {"AUTO"};              // Флаг автоматической подбора частоты
...
const char *fc_UPTIME            = {"UPTIME"};            // Время обновления алгоритма пид регулятора (сек) Основной цикл управления
const char *fc_PID_FREQ_STEP     = {"PID_FREQ_STEP"};     // Максимальный шаг (на увеличение) изменения частоты при ПИД регулировании в 0.01 Гц Необходимо что бы ЭРВ успевал
const char *fc_PID_STOP          = {"PID_STOP"};          // Проценты от уровня защит (мощность, ток, давление, темпеартура) при которой происходит блокировка роста частоты пидом
...
const char *fc_START_FREQ        = {"START_FREQ"};        // Стартовая частота инвертора (см компрессор) в 0.01 ГЦ
const char *fc_START_FREQ_BOILER = {"START_FREQ_BOILER"}; // Стартовая частота инвертора (см компрессор) в 0.01 ГЦ ГВС
const char *fc_MIN_FREQ          = {"MIN_FREQ"};          // Минимальная  частота инвертора (см компрессор) в 0.01 Гц
const char *fc_MIN_FREQ_COOL     = {"MIN_FREQ_COOL"};     // Минимальная  частота инвертора при охлаждении в 0.01 Гц
const char *fc_MIN_FREQ_BOILER   = {"MIN_FREQ_BOILER"};   // Минимальная  частота инвертора при нагреве ГВС в 0.01 Гц
const char *fc_MIN_FREQ_USER     = {"MIN_FREQ_USER"};     // Минимальная  частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 Гц
const char *fc_MAX_FREQ          = {"MAX_FREQ"};          // Максимальная частота инвертора (см компрессор) в 0.01 Гц
const char *fc_MAX_FREQ_COOL     = {"MAX_FREQ_COOL"};     // Максимальная частота инвертора в режиме охлаждения  в 0.01 Гц
const char *fc_MAX_FREQ_BOILER   = {"MAX_FREQ_BOILER"};   // Максимальная частота инвертора в режиме ГВС в 0.01 Гц поглощение бойлера обычно меньше чем СО
const char *fc_MAX_FREQ_USER     = {"MAX_FREQ_USER"};     // Максимальная частота инвертора РУЧНОЙ РЕЖИМ (см компрессор) в 0.01 Гц
const char *fc_STEP_FREQ         = {"STEP_FREQ"};         // Шаг уменьшения инвертора при достижении максимальной температуры, мощности и тока (см компрессор) в 0.01 Гц
const char *fc_STEP_FREQ_BOILER  = {"STEP_FREQ_BOILER"};  // Шаг уменьшения инвертора при достижении максимальной температуры, мощности и тока ГВС в 0.01 Гц
...
const char *hp_TEMP_IN   = {"TEMP_IN"};          // температура подачи (минимальная)
const char *hp_TEMP_OUT  = {"TEMP_OUT"};         // температура обратки (максимальная)
