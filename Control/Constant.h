...
#define VERSION         "0.945 beta"        // Версия прошивки
#ifndef UART_SPEED
#define UART_SPEED       115200             // Скорость отладочного порта
#endif
#define DEMO                                // Включение демо режима
#define DEBUG                               // В последовательный порт шлет сообщения в первую очередь ошибки
//#define STAT_FREE_RTOS                      // НЕ ИСПОЛЬЗОВАТЬ ЗДЕСЬ Возможность посмотреть статистику free RTOS жрет ресурсы устанавливать в FreeRTOSConfig.h
...
#define VER_SAVE          121               // Версия формата сохраняемых данных, при чтении если версии не совпадают, отказ от чтения. ВСЕГДА 2 БАЙТА тип uint16_t
...
#define HEAT_CAPACITY     4174           // теплоемкость жидкости в конутре СО при 30 градусах [Cp, Дж/(кг·град)]
#define KF_HEAT_CAPACITY  (float)(3600*100/HEAT_CAPACITY)   // Коэффициент пересчета для определениея мощности  (3600 секунды в часе) в СОТЫХ!!!
...
#define ERR_SD_INIT        -35         // Ошибка иннициализации SD карты
...
enum BOILER_HP          
{
    ...
    pBOIL_TEMP,                    // Целевая темпеартура ПИД ГВС
...
//  Перечисляемый тип - параметры ТН
enum PARAM_HP       
{
...
enum TYPE_CHART       
{
    ...
    pFLOWCON,                  // Датчик потока по кондесатору
    pFLOWEVA,                  // Датчик потока по испарителю
    pFLOWPCON,                 // Датчик протока по предконденсатору
    pposEEV,                   // позиция ЭРВ
    pfreqFC,                   // Частота инвертора
    ppowerFC,                  // Мощность инвертора
    pcurrentFC,                // Ток компрессора
    pRCOMP,                    // включение компрессора
    pOVERHEAT,                 // перегрев
    pdCO,                      // дельта СО
    pdGEO,                     // дельта геоконтура
    pTPEVA,                    // температура расчитанная из давления Испариенифя
    pTPCON,                    // температура расчитанная из давления Конденсации
    pPowerCO,                  // Мощность выходная теплового насоса
    pPowerGEO,                 // Мощность контура
    pCOP,                      // Коэффициент преобразования Холодильной машины (без насосов)
    pVOLTAGE,                  // Статистика по напряжению
    pCURRENT,                  // Статистика по току
    pacPOWER,                  // Статистика по активная мощность
    prePOWER,                  // Статистика по Реактивная мощность
    pfullPOWER,                // Статистика по Полная мощность
    pkPOWER,                   // Статистика по Коэффициент мощности
    pfullCOP,                  // Полный COP
