...
#define VERSION         "0.994 beta"        // Версия прошивки
...
#define CHART_POINT       300               // Максимальное число точек графика //300 - работает
#ifndef ADC_PRESCAL
...
#define FC_NUM_READ        4             // Число попыток чтения инвертора (подряд) по модбас до его останова ТН по ошибке
...
#define SDM_DELAY_REPEAD    40             // мсек Время между ПОВТОРНЫМИ попытками чтения                      было 150
...
// 0х0080 - I2C_SETTING_EEPROM хранение настроек ТН  максимальный размер 0х980 (2432) байт. Сейчас используется 1080 байт
...
#define ERR_DEFROST_RTRV    -81        // Ошибочная конфигурация - попытка разморозки при не возможности переключения на охлаждение (нет RTRV)
...
const char *eev_trend_threshold = {"TTH"};			// trend_threshold
...
const char *boil_fBoilerPID   = {"PID"};
...
const char *chart_TPEVA     = {"T[PEVA]"};                   // Температура расчитанная из давления Испариения
const char *chart_TPCON     = {"T[PCON]"};                   // Температура расчитанная из давления Конденсации
const char *chart_TCON      = {"TCONDES"};                   // Температруа Конденсации (вариант без датчика давления)
const char *chart_PowerCO   = {"PowerCO"};                   // Мощность выходная теплового насоса
const char *chart_PowerGEO  = {"PowerGEO"};                  // Мощность ГЕО контура
const char *chart_COP       = {"COP"};                       // Коэффициент преобразования Холодильной машины (без насосов)
const char *chart_VOLTAGE   = {"VOLTAGE"};                   // Напряжению (счетчик SDM)
const char *chart_CURRENT   = {"CURRENT"};                   // Ток (счетчик SDM)
//const char *chart_acPOWER   = {"acPOWER"};                   // Активная мощность
//const char *chart_rePOWER   = {"rePOWER"};                   // Реактивная мощность
const char *chart_fullPOWER = {"fullPOWER"};                 // Полная мощность (счетчик SDM)
...
const char *noteError[] = {"Ok",                                                  //  0
                           ...
                           "Попытка разморозки при не возможности переключения на охлаждение (нет RTRV)",       //-81
