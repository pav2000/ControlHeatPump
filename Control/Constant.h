...
#define VERSION         "0.984 beta"        // Версия прошивки
#define VER_SAVE		129					// Версия формата сохраняемых данных в I2C память
#ifndef UART_SPEED
...
struct PID_STRUCT {   		// Настройки ПИД регулятора
	  ...
	  int16_t Kp;           // ПИД Коэф. пропорц, в сотых для алгоритма 1, в тысячных для алгоритма 2
	  int16_t Ki;           // ПИД Коэф. интегральный, в сотых для алгоритма 1, в тысячных для алгоритма 2
	  int16_t Kd;           // ПИД Коэф. дифференциальный, в сотых для алгоритма 1, в тысячных для алгоритма 2
	  int16_t Kp_dmin;      // Разница (в сотых градуса) при которой происходит уменьшение пропорциональной составляющей ПИД ЭРВ, либо выбор других коэффициентов для PID2
};

struct PID_WORK_STRUCT {    // Переменные ПИД регулятора
	int32_t sum;			// сумма для интегрирования
	int32_t pre_errPID;		// предыдущая ошибка для дифференцирования
