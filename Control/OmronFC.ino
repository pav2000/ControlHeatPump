...
{
	journal.jprintf("Programming Omron MX2 %s . . .\r\n",name);
	...
	journal.jprintf("Initialization . . .\r\n"); 
    _delay(7000);
	progReg16(MX2_H102,"H102",0x01);   // H102 Установка кода PM-двигателя  00 (стандартные данные Omron) 01 (данные автонастройки) = 1
	progReg16(MX2_H103,"H103",7);      // H103 Мощность PM-двигателя (0,1/0,2/0,4/0,55/0,75/1,1/1,5/2,2/3,0/3,7/4,0/5,5/7,5/11,0/15,0/18,5) = 7
	progReg16(MX2_H104,"H104",0x04);   // H104 Установка числа полюсов PM двигателя = 4
	progReg16(MX2_H105,"H105",1000);   // H105 Номинальный ток PM-двигателя = 1000 (это 11А)
	progReg16(MX2_H106,"H106",550);    // H106 Константа R PM-двигателя От 0,001 до 65,535 Ом =0.55 *1000
	progReg16(MX2_H107,"H107",231);    // H107 Константа Ld PM-двигателя От 0,01 до 655,35 мГн =2.31*100
	progReg16(MX2_H108,"H108",270);    // H108 Константа Lq PM-двигателя От 0,01 до 655,35 мГн =2.7*100
	progReg16(MX2_H109,"H109",750);    // H109 Константа Ke PM-двигателя 0,0001...6,5535 Вмакс./(рад/с) =750 надо подбирать влияет на потребление и шум
    progReg32(MX2_H110,"H110",10);     // H110 Константа J PM-двигателя От 0,001 до 9999,000 кг/мІ =0.01
	progReg16(MX2_H119,"H119",100);    // H119 Постоянная стабилизации PM двигателя От 0 до 120% с =100
	progReg16(MX2_H121,"H121",10);     // H121 Минимальная частота PM двигателя От 0,0 до 25,5%  =10 (default)
	progReg16(MX2_H122,"H122",10);     // H122 Ток холостого хода PM двигателя От 0,00 до 100,00%   =50 (default)
	#ifndef DEMO   // для демо не надо термозащиты настраивать
	progReg16(MX2_C005,"C005",19);     // C005 Функция входа [5] [также вход «PTC»]   = 19 PTC Термистор с положительным ТКС для тепловой защиты (только C005)
	#endif
	progReg16(MX2_b091,"b091",1);      // b091 Выбор способа остановки 0 (торможение до полной остановки),1 (остановка выбегом)=1
	progReg16(MX2_b021,"b021",1);      // b021 Режим работы при ограничении перегрузки =1
	progReg16(MX2_b022,"b022",1000);   // b022 Уровень ограничения перегрузки  200...2000 (0.1%) =                                      
	progReg16(MX2_b023,"b023",10);     // b023  Время торможения при ограничении перегрузки (0.1 сек)=10
	progReg16(MX2_A001,"A001",3);      // A001 Источник задания частоты =03
	progReg16(MX2_A002,"A002",1);      // A002 Источник команды «Ход» 
	progReg16(MX2_A003,"A003",FC_BASE_FREQ/10);     // A003 A003=60  основная частота
