...
#define fCorrectOverHeat 4		// Включен режим корректировки перегрева
...
public:
 ...
 // int8_t Stop();                                       // Останов ЭРВ - завершение алгоритма отслеживания
  ...
  int16_t get_timeIn(){ return  timeIn;}                 // Получить ЗАДАННУЮ постоянную интегрирования времени в секундах ЭРВ
  int8_t  set_timeIn(int16_t t);                         // Установить постоянную интегрирования времени в секундах ЭРВ
  int16_t get_delay() {return tmpTime;}                  // Получить ТЕКУЩУЮ постоянную интегрирования времени в секундах ЭРВ
  int16_t get_Kpro()  { return  Kp;}                     // Пропорциональная составляющая  В СОТЫХ!!!
  int8_t  set_Kpro(int16_t x);                           // Установить Коэф пропорц  В СОТЫХ!!!
  int16_t get_Kint(){return  Ki;}                        // Интегральная составляющая  В СОТЫХ!!!
  int8_t  set_Kint(int16_t x);                           // Установить Коэф интегр. В СОТЫХ!!!
  int16_t get_Kdif(){return  Kd;}                        // Дифференциальная составляющая  В СОТЫХ!!!
  int8_t  set_Kdif(int16_t x);                           // Установить Коэф дифф. В СОТЫХ!!!
  int16_t get_Correction(){return Correction;}           // Получить поправку в градусах для правила работы ЭРВ «TEVAOUT-TEVAIN».  СОТЫЕ ГРАДУСА
  int8_t  set_Correction(int16_t x);                     // Установить поправку в градусах для правила работы ЭРВ «TEVAOUT-TEVAIN».
  int16_t get_manualStep(){return manualStep;}           // Получить число шагов открытия ЭРВ для правила работы ЭРВ «Manual»
  int8_t  set_manualStep(int16_t x);                     // установить число шагов открытия ЭРВ для правила работы ЭРВ «Manual»
  TYPEFREON get_typeFreon(){return typeFreon;}           // Получить тип фреона
  void set_typeFreon(TYPEFREON  f){typeFreon=f;}         // Установить тип фреона
  RULE_EEV get_ruleEEV(){return ruleEEV;}                // Получить тип фреона
  void set_ruleEEV(RULE_EEV  r){ruleEEV=r;}              // Установить тип фреона
  ...
  int16_t get_minEEV(){return  minEEV;}                  // Число шагов при котором ЭРВ начинает открываться (холостой ход) может быть равным 0
//  boolean get_present();                               // Наличие ЭРВ текущей конфигурации
  __attribute__((always_inline)) inline boolean get_present(){return GETBIT(flags,fPresent);} // Наличие EEV в текущей конфигурации
  int32_t save(int32_t adr);                             // Записать настройки в eeprom i2c на входе адрес с какого, на выходе конечный адрес, число меньше 0 это код ошибки
  int32_t load(int32_t adr);                             // Считать настройки из eeprom i2c на входе адрес с какого, на выходе конечный адрес, число меньше 0 это код ошибки
  int32_t loadFromBuf(int32_t adr,byte *buf);            // Считать настройки из буфера на входе адрес с какого, на выходе конечный адрес, число меньше 0 это код ошибки
  uint16_t get_crc16(uint16_t crc);                      // Рассчитать контрольную сумму для данных на входе входная сумма на выходе новая
  ...
  int16_t timeIn;                                        // !save! Постоянная интегрирования времени в секундах ЭРВ СЕКУНДЫ
  int16_t tOverheat;                                     // !save! Перегрев ЦЕЛЬ (сотые градуса)
  int16_t Kp ;                                           // !save! ПИД Коэф пропорц.  В СОТЫХ!!!
  int16_t Ki ;                                           // !save! ПИД Коэф интегр.  для настройки Ki=0  В СОТЫХ!!!
  int16_t Kd ;                                           // !save! ПИД Коэф дифф.   В СОТЫХ!!!
  int16_t Correction;                                    // !save! 0.855 ПЕРЕДЕЛАНО  зона не чуствительности перегрева в "плюсе" в этой зоне на каждом шаге эрв закрывается на 1 шаг
  int16_t manualStep;                                    // !save! Число шагов открытия ЭРВ для правила работы ЭРВ «Manual»
  TYPEFREON typeFreon;                                   // !save! Тип фреона
  RULE_EEV ruleEEV;                                      // !save! правило работы ЭРВ
  struct {
  ...
  uint16_t EEV_KP_ERR;            // Ошибка (в сотых градуса) при которой происходит уменьшение пропорциональной составляющей ПИД ЭРВ
  uint8_t  EEV_SPEED;             // Скорость шагового двигателя ЭРВ (импульсы в сек.)
  uint16_t EEV_PSTART;            // ПУСКОВАЯ позиция ЭРВ (ТО что при старте компрессора ПРИ РАСКРУТКЕ)
  uint16_t EEV_START;             // СТАРТОВАЯ позиция ЭРВ после раскрутки компрессора т.е. ПОЗИЦИЯ С КОТОРОЙ НАЧИНАЕТСЯ РАБОТА проходит DELAY_START_POS сек
  uint16_t EEV_MIN_STEPS;         // Минимальное число шагов открытия ЭРВ
  uint8_t  EEV_HOLD_MOTOR;        // Режим "удержания" шагового двигателя ЭРВ
  // ЭРВ Времена и задержки
  uint8_t  DELAY_ON_PID_EEV;      // Задержка включения EEV после включения компрессора (сек).  Точнее после выхода на рабочую позицию Общее время =DELAY_ON_PID_EEV+DELAY_START_POS
  uint8_t  DELAY_ON3_EEV;         // Задержка между открытием (для старта) ЭРВ и включением компрессора, для выравнивания давлений (сек). Если ЭРВ закрывлось при остановке
  uint8_t  DELAY_START_POS;       // Время после старта компрессора когда EEV выходит на стартовую позицию - облегчение пуска вначале ЭРВ
  uint8_t  DELAY_OFF_EEV;         // Задержка закрытия EEV после выключения насосов (сек). Время от команды стоп компрессора до закрытия ЭРВ = DELAY_OFF_PUMP+DELAY_OFF_EEV
...
private:
  ...
  int16_t halfPos;                                       // Позиция шаговика - половина диапазона ЭРВ
